...
@rem Android Gradle Plugin 8+ requires JDK 17+. Prefer ANDROID_GRADLE_JDK, then Android Studio JBR, if JAVA_HOME is not 17+.
if not "%ANDROID_GRADLE_JDK%"=="" if exist "%ANDROID_GRADLE_JDK%\bin\java.exe" (
  set "JAVA_HOME=%ANDROID_GRADLE_JDK%"
  goto afterJnBootstrap
)
set "_JN_OK="
if defined JAVA_HOME if exist "%JAVA_HOME%\bin\java.exe" (
  "%JAVA_HOME%\bin\java.exe" -version 2>&1 | findstr /R /C:"version \"1[7-9]\." /C:"version \"[2-9][0-9]\." >nul
  if not errorlevel 1 set "_JN_OK=1"
)
if "%_JN_OK%"=="1" goto afterJnBootstrap
