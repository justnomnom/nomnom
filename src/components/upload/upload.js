...
export default function Upload({
  ...
  const renderPlaceholder = (
    <Stack spacing={3} alignItems="center" justifyContent="center" flexWrap="wrap">
      ...
      <Stack spacing={1} sx={{ textAlign: 'center' }}>
        <Typography variant="h6">{t('components.upload.drop_or_select_title')}</Typography>
        <Typography variant="body2" sx={{ color: 'text.secondary' }}>
          <Trans
            i18nKey="components.upload.drop_hint"
            components={{
              browse: (
                <Box
                  component="span"
  ...
  const removeSinglePreview = hasFile && onDelete && (
    <IconButton
      onClick={onDelete}
      aria-label={t('common.a11y.remove_file')}
      sx={{
        top: 8,
        right: 8,
        zIndex: 9,
    ...
    >
      <Iconify icon={ic.closeLine} width={20} />
    </IconButton>
  );
